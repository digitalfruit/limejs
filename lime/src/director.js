 ...
 * @param {number=} opt_width Optionaly define what height and width
 * the director should have.
 * @param {number=} opt_height Optionaly define what height and width
 * the director should have.
 * @constructor
 * @extends {lime.Node}
 */
lime.Director = function(parentElement, opt_width, opt_height) {
  ...
  this.setSize(new goog.math.Size(
      ...
      arguments[2] || parentSize.height * width / parentSize.width ||
 ...
 * @param {lime.transitions.Transition=} opt_transition Transition played.
 * @param {number=} opt_duration Duration of transition.
 * @return {lime.transitions.Transition} object if opt_transition is defined.
 */
lime.Director.prototype.popScene = function(opt_transition, opt_duration) {

  var outgoing = this.getCurrentScene();

  if (goog.isNull(outgoing)) return null;
...
lime.Director.prototype.keyUpHandler_ = function(e) {
  if (e.altKey && String.fromCharCode(e.keyCode).toLowerCase() == 'd') {
    if (this.debugModeOn_) {
      ...
      this.debugModeOn_ = goog.style.installStyles(
          '.lime-scene div,.lime-scene img,' +
